...
public class NetworkManager : MonoBehaviourPunCallbacks
{
    ...
    public Sprite GameModeOneBackground;
    public Sprite GameModeTwoBackground;
    ...
    public void OnJoinRandomRoomClicked(string _gameMode)
    {
        GameMode = _gameMode;

        ExitGames.Client.Photon.Hashtable exptextedCustomRoomProperties = new ExitGames.Client.Photon.Hashtable() { { "gm", _gameMode } };
        PhotonNetwork.JoinRandomRoom(exptextedCustomRoomProperties, 0);
    }
    ...
    public void OnStartGameButtonClicked()
    {
        ...
        {
            if(PhotonNetwork.CurrentRoom.CustomProperties.ContainsValue("One"))
            {
                //One Gamemode
                PhotonNetwork.LoadLevel("Scene1");
            }
            else if (PhotonNetwork.CurrentRoom.CustomProperties.ContainsValue("Two"))
            {
                //GameMode two'
    ...
    public override void OnJoinedRoom()
    {
        ...
        if(PhotonNetwork.CurrentRoom.CustomProperties.ContainsKey("gm"))
        {
            RoomInfoText.text = "Room name: " + PhotonNetwork.CurrentRoom + " " + " Players/Max.Players: " + PhotonNetwork.CurrentRoom.PlayerCount + " / " + PhotonNetwork.CurrentRoom.MaxPlayers;


            if (PhotonNetwork.CurrentRoom.CustomProperties.ContainsValue("One"))
            {
                //Gamemode ONE!
                GameModeText.text = "Gamemode ONE";
                PanelBackground.sprite = GameModeOneBackground;
            }
            else if (PhotonNetwork.CurrentRoom.CustomProperties.ContainsValue("Two"))
            {
                //Gamemode TWO!
                GameModeText.text = "Gamemode TWO";
                PanelBackground.sprite = GameModeTwoBackground;
            }
            ...
            foreach(Player player in PhotonNetwork.PlayerList)
            {
                ...
                if(player.CustomProperties.TryGetValue(MultiplayerGame.PLAYER_READY, out isPlayerReady))
                {
    ...
    public override void OnPlayerPropertiesUpdate(Player targetPlayer, ExitGames.Client.Photon.Hashtable changedProps)
    {
        ...
        {
            object isPlayerReady;
            if(changedProps.TryGetValue(MultiplayerGame.PLAYER_READY, out isPlayerReady))
            {
                playerListGameObject.GetComponent<PlayerListEntryInitializer>().SetPlayerReady((bool)isPlayerReady);
            }
        }
